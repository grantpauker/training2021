...
class Trajectories:
    MAX_VELOCITY = 16 * (units.feet / units.seconds)
    MAX_ACCELERATION = 8 * (units.feet / units.seconds / units.seconds)
    MAX_CENTRIPETAL_ACCELERATION = 8 * (units.ft / units.s / units.s)
    MAX_VOLTAGE = 10 * units.volts
    ...
    def __init__(self):
        ...
        self.shoot_to_rendezvous_two_balls = TrajectoryGenerator.generateTrajectory(
            (
                ...
                waypoints.Waypoints.RENDEZVOUS_TWO_BALLS,
            ),
            self.rev_config,
        )
        ...
        self.rendezvous_two_balls_to_shoot = TrajectoryGenerator.generateTrajectory(
            (
                waypoints.Waypoints.RENDEZVOUS_TWO_BALLS,
                waypoints.Waypoints.IDEAL_SHOOT,
            ),
            self.fwd_config,
        )
